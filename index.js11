...
schedule.scheduleJob(dailyUpdateRule, async () => {
  try {
    ...
    for (const car of cars) {
      ...
      Logger.database(`Updated insurance: ${car.carName} (${car.numberPlate}) - New expiry: ${newExpiry.format('DD MMM YYYY')}`, 'UPDATE');
    }
      ...
      try {
        ...
        for (const [index, chunk] of carChunks.entries()) {
          const embed = new EmbedBuilder()
            ...
            .setFooter({ 
              text: `Alert generated by ${interaction.user.tag} • ${moment().tz('Asia/Kolkata').format('DD MMM YYYY hh:mm A')}`,
              iconURL: 'https://cdn.discordapp.com/attachments/1395239890588799128/1407301018588545144/file_00000000bea8622f9b87bdfc3ad9115e.png?ex=68a59ab5&is=68a44935&hm=3668c48fbf895816ce1cf7ae4f6206c5c02e6f849d9db59bbc2c82ebdaad98a3&'
            });
